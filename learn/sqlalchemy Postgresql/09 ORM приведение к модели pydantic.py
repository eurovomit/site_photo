...
# ------------- приведение к модели Pydantic без relationships -----------------
...
with sync_session_factory() as session:
    ...
    print(result)
    res_dto = [WorkersDTO.model_validate(row, from_attributes=True) for row in result]
    print(res_dto)

# ------------- приведение к модели Pydantic с relationships -----------------
...
with sync_session_factory() as session:
    ...
    result = res.scalars().all()
    print(result)
    res_dto = [WorkersRelDTO.model_validate(row, from_attributes=True) for row in result]
    print(res_dto)